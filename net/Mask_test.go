...
type maskBytePair struct {
	mask []byte
	b    byte
}

var maskBytePairs []maskBytePair

func init() {
	//Exhaustive
	maskBytePairs = []maskBytePair{
		{[]byte{0x00, 0, 0, 0}, 0},
		{[]byte{0x80, 0, 0, 0}, 1},
		{[]byte{0xC0, 0, 0, 0}, 2},
		{[]byte{0xE0, 0, 0, 0}, 3},
...
func TestToByte(t *testing.T) {
	for i := 0; i < len(maskBytePairs); i++ {
		mask := net.IPMask(maskBytePairs[i].mask)
		found := MaskToByte(mask)
		expected := maskBytePairs[i].b
...
func TestFromByte(t *testing.T) {
	for i := 0; i < len(maskBytePairs); i++ {
		...
		expected := net.IPMask(maskBytePairs[i].mask)
		found := MaskFromByte(maskBytePairs[i].b)

		if !bytes.Equal(found,expected) {
			t.Fatalf("Expecting mask from byte %v to be %v, got %v", maskBytePairs[i].b, expected, found)
		}
	}
}