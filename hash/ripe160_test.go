...
type ripe160pair struct {
	s   string
	hex string
}

var ripe160pairs []ripe160pair

func init() {
	ripe160pairs = []ripe160pair{
...
func TestRipe160(t *testing.T) {
	for i := 0; i < len(ripe160pairs); i++ {
		...
		if foundHex != rec.hex {
			t.Fatalf("Ripe160 %v, expecting %v, got %v", abbr(rec.s), rec.hex, foundHex)
		}
	}
}
