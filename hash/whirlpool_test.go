...
type whirlpoolPair struct {
	s   string
	hex string
}

var whirlpoolPairs []whirlpoolPair

func init() {
	whirlpoolPairs = []whirlpoolPair{
		...
		{strings.Repeat("a",1e6),"0C99005BEB57EFF50A7CF005560DDF5D29057FD86B20BFD62DECA0F1CCEA4AF51FC15490EDDC47AF32BB2B66C34FF9AD8C6008AD677F77126953B226E4ED8B01"},
		//Others
...
func TestWhirlpool(t *testing.T) {
	for i := 0; i < len(whirlpoolPairs); i++ {
		rec := whirlpoolPairs[i]
		d := whirlpool.New()
		d.Write([]byte(rec.s))
		found := d.Sum([]byte{})
		...
		if foundHex != rec.hex {
			t.Fatalf("Whirlpool %v, expecting %v, got %v", abbr(rec.s), rec.hex, foundHex)
		}
	}
}
